...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetGameManaegerNetworkObject = null;
		public GameObject[] NetworkCameraNetworkObject = null;
		public GameObject[] TestNetworkObject = null;
		public GameObject[] NetTileNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetGameManaegerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetGameManaegerNetworkObject.Length > 0 && NetGameManaegerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetGameManaegerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is NetTileNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetTileNetworkObject.Length > 0 && NetTileNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetTileNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetGameManaeger instead, its shorter and easier to type out ;)")]
		public NetGameManaegerBehavior InstantiateNetGameManaegerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetGameManaegerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetGameManaegerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetGameManaegerBehavior>().networkObject = (NetGameManaegerNetworkObject)obj;
		...
		[Obsolete("Use InstantiateNetTile instead, its shorter and easier to type out ;)")]
		public NetTileBehavior InstantiateNetTileNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetTileNetworkObject[index]);
			var netBehavior = go.GetComponent<NetTileBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetTileBehavior>().networkObject = (NetTileNetworkObject)obj;
		...
		public NetGameManaegerBehavior InstantiateNetGameManaeger(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetGameManaegerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetGameManaegerBehavior>();
		...
		public NetTileBehavior InstantiateNetTile(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetTileNetworkObject[index]);
			var netBehavior = go.GetComponent<NetTileBehavior>();
